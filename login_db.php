...
if (isset($_POST['submit'])) {
    ...
    // กรณีที่กรอกข้อมูลครบถ้วนจะทำการ query ข้อมูล เพื่อเช็คว่ามี username นี้อยู่ในระบบหรือไม่
        ...
        $select_stmt = $db->prepare("SELECT COUNT(username) AS count_uname, password,role,role.id as roleId FROM users  LEFT JOIN role ON role.id = users.role WHERE username = :username");
        ...
        // ถ้าไม่พบ username จะทำการตรวจสอบ password โดยเทียบ password ที่กรอกเข้ามาตรงกับ password ใน database หรือไม่ ผ่านฟังก์ชัน password_verify() ถ้าตรงกันเงื่อนไขจะเป็นจริง
            ...
            if (password_verify($password, $row['password'])) {
                ...
                $sql = $db->query("SELECT * FROM permission WHERE role_id = '$roleId'");
                $sql->execute();
                $stmt = $sql->fetchAll();
            
                $permissionList = json_encode($stmt, true);

                print $permissionList;
                $_SESSION['username'] = $username;
                $_SESSION['is_logged_in'] = true;
                $_SESSION['role'] = $row['role'];
                $_SESSION['permission'] = $permissionList;
