...
<body>
    ...
    <div class="container mt-5">
        ...
        <form action="EditOutstanding_db.php" method="POST" enctype="multipart/form-data">
            ...
            if (isset($_GET['id'])) {
                ...
                $sql = $db->query("SELECT* FROM tbl_outstandingemployee WHERE id = $id");
                $sql->execute();
                $stmt = $sql->fetch();
            } //isset
            ?>

            <h1>แก้ไขข้อมูลพนักงานดีเด่น</h1>
            <hr>
            <p>ป้อนข้อมูลพนักงานดีเด่นด้านงานบริการที่ต้องการแก้ไขในฐานข้อมูล</p>
            <div class="mb-3">
                ...
                <input type="hidden" class="form-control" value="<?= $stmt['img_employee']; ?>" name="img2_employee" require>
            </div>
            ...
            <div class="mb-3 mt-3">
                ...
                <input type="file" class="form-control" placeholder="รูปภาพ" id="imgInput" name="img_employee"></br>
                <img src="upload_img_employeeoutstanding/<?= $stmt['img_employee']; ?>" width="100px" id="PreviewImg" class="rounded">
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control " value="<?= $stmt['services']; ?>" placeholder="" name="services" >
            </div>

            <div class="mb-3 mt-3">
                <label for="title_employee" class="form-label">ชื่อ-นามสกุล</label>
                <input type="text" class="form-control" value="<?= $stmt['title_employee']; ?>" placeholder="" name="title_employee" >
            </div>

            <div class="mb-3 mt-3">
                <label for="details_employee" class="form-label">รายละเอียด</label>
                <input type="text" class="form-control " value="<?= $stmt['details_employee']; ?>" placeholder="" name="details_employee"></input>
            </div>

            <div class="mb-3 mt-3">
                <label for="position" class="form-label">ตำแหน่ง</label>
                <input type="text" class="form-control " value="<?= $stmt['position']; ?>" placeholder="" name="position" >
            </div>

            <div class="mb-3 mt-3">
                <a class="btn btn-secondary btn-sm" href="admin.php">กลับ</a>
                <button id="edit" type="submit" name='update' class="btn btn-sm">อัพเดตข้อมูล</button>
            </div>
        </form>
        <hr>
